    ...
    <>
      <header className="h-12 bg-[#000500] border-b border-[#00FF41]/30 px-4 flex items-center justify-between font-mono text-[#00FF41]">
        ...
        <div className="flex items-center gap-4">
          <div className="text-xs">{live ? 'LIVE IN THE FIELD OF REEDS' : 'OFFLINE LOCK'} · CASE {status?.activeCaseId ?? 'n/a'} · BORDERS {status?.counts?.borders ?? 0} · HIGH-ENTROPY {status?.counts?.highEntropyNodes ?? 0}</div>
          <button
