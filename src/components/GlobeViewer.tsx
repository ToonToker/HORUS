...
const GlobeViewer = () => {
  ...
  const loadGeoCollection = async (items: Track[], color: Color) => {
    ...
    return GeoJsonDataSource.load(geojson, { stroke: color, fill: color.withAlpha(0.06), strokeWidth: 2, clampToGround: true });
  };
  ...
  return (
    <div className="w-full h-full relative">
      <Viewer ref={viewerRef} full terrainProvider={undefined} baseLayerPicker={false} timeline={false} animation={false} geocoder={false} homeButton={false} sceneModePicker={false} navigationHelpButton={false} imageryProvider={new SingleTileImageryProvider({ url: '/data/tactical-basemap.svg' })}>
        ...
        {layers.threatMap && threatArcs.map((a) => <Entity key={a.id} polyline={{ positions: Cartesian3.fromDegreesArray([a.from?.lon ?? 0, a.from?.lat ?? 0, a.to?.lon ?? 0, a.to?.lat ?? 0]), width: 2, material: Color.fromCssColorString('#FF3131'), arcType: ArcType.GEODESIC }} properties={a} />)}
        {layers.rfNodes && recent(rfNodes).map((r) => <Entity key={r.id} position={Cartesian3.fromDegrees(r.lon ?? 0, r.lat ?? 0)} point={{ pixelSize: 5, color: Color.AQUA }} label={{ text: r.name ?? 'RF', fillColor: Color.AQUA, font: '10px monospace', pixelOffset: new Cartesian2(0, -12) }} properties={r} />)}
        {layers.maritime && recent(vessels).map((v) => <Entity key={v.id} position={Cartesian3.fromDegrees(v.lon ?? 0, v.lat ?? 0)} point={{ pixelSize: 6, color: Color.CYAN }} label={{ text: `${v.callsign ?? 'VSL'} ${Math.round(v.speed ?? 0)}kt`, fillColor: Color.CYAN, font: '10px monospace', pixelOffset: new Cartesian2(0, -12) }} properties={v} />)}
        {layers.cyberThreats && recent(cyberThreats).map((c) => <Entity key={c.id} position={Cartesian3.fromDegrees(c.lon ?? 0, c.lat ?? 0)} point={{ pixelSize: 4 + (c.intensity ?? 1), color: Color.fromCssColorString('#FF3131') }} properties={c} />)}
        {layers.signalFog && wardriving.map((w) => <Entity key={w.id} position={Cartesian3.fromDegrees(w.lon ?? 0, w.lat ?? 0)} point={{ pixelSize: 4 + Math.min(8, (w.density ?? 1) / 2), color: Color.fromCssColorString('#00FF41').withAlpha(0.35) }} properties={w} />)}
        {layers.resonanceLinks && resonanceLinks.map((l) => <Entity key={l.id} polyline={{ positions: Cartesian3.fromDegreesArray([l.from?.lon ?? 0, l.from?.lat ?? 0, l.to?.lon ?? 0, l.to?.lat ?? 0]), width: 1.5, material: Color.fromCssColorString('#FFD700') }} properties={l} />)}
        {layers.ghostMarkers && ghostMarkers.map((g) => <Entity key={g.id} position={Cartesian3.fromDegrees(g.lon ?? 0, g.lat ?? 0)} point={{ pixelSize: 6, color: Color.WHITE.withAlpha(0.45) }} label={{ text: g.name ?? 'Ghost', fillColor: Color.WHITE, font: '10px monospace', pixelOffset: new Cartesian2(0, -10) }} properties={g} />)}
      </Viewer>
      <div className="absolute left-3 bottom-3 text-xs text-emerald-300 bg-black/70 border border-emerald-600/50 p-2 rounded">DJED-STABILIZER ACTIVE · Hover/click vectors for metadata.</div>
    </div>
  );
};
