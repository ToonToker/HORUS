...
const Sidebar = () => {
  const { layers, toggleLayer, pendingWitnessPoint, selectedEntity } = useWorldViewStore();
  ...
  return (
    <aside className="w-84 h-full bg-[#000500] text-[#00FF41] border-r border-[#00FF41]/30 p-4 overflow-y-auto font-mono">
      ...
      <div className="mt-6 border border-[#FFD700]/40 rounded p-3">
        ...
        <button className="w-full mt-2 border border-[#FFD700] text-[#FFD700] py-1 font-bold" onClick={runDeepScrape}>OSINT-SCRAPE</button>
        <button className="w-full mt-2 border border-[#00FF41] text-[#00FF41] py-1 font-bold" onClick={ingestSeekerNode}>Inject Seeker Node</button>
